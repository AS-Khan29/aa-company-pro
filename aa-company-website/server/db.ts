...
import { InsertUser, users, contactSubmissions, InsertContactSubmission, quoteRequests, InsertQuoteRequest } from "../drizzle/schema";
...
export async function getContactSubmissions(limit = 50) {
  ...
  try {
    const result = await db.select().from(contactSubmissions).orderBy(desc(contactSubmissions.createdAt)).limit(limit);
    return result;
  } catch (error) {
...
export async function getQuoteRequests(limit = 50) {
  ...
  try {
    const result = await db.select().from(quoteRequests).orderBy(desc(quoteRequests.createdAt)).limit(limit);
    return result;
  } catch (error) {
